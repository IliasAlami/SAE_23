...
nombre_batiment=$(echo "SELECT COUNT(id_batiment) FROM sae23.batiment;" | /opt/lampp/bin/mysql -h localhost -u asuisse -pJuloto31 2>/dev/null| tail -1)

# Loop through each building
for ((i=0; i<$nombre_batiment; i++)); do
	
	# Retrieve the id_batiment for the current iteration
	batiment=$(echo "SELECT id_batiment FROM sae23.batiment LIMIT $i,1;" | /opt/lampp/bin/mysql -h localhost -u asuisse -pJuloto31  2>/dev/null| tail -1)

	##echo  batiment $batiment
	
	# Retrieve the total number of rooms in the sae23.capteur table in a building
	nombre_room=$(echo "SELECT COUNT(salle) FROM sae23.capteur WHERE id_batiment=${batiment};" | /opt/lampp/bin/mysql -h localhost -u asuisse -pJuloto31 2>/dev/null| tail -1)

	# Loop through each room in the current building
	for ((x=0; x<$nombre_room; x++)); do

		# Retrieve the salle value for the current building and room
		room=$(echo "SELECT salle FROM sae23.capteur WHERE id_batiment=${batiment} LIMIT $x,1;" | /opt/lampp/bin/mysql -h localhost -u asuisse -pJuloto31  2>/dev/null| tail -1)
		...
		typeCapt=$(echo "SELECT type FROM sae23.capteur WHERE salle='${room}';" | /opt/lampp/bin/mysql -h localhost -u asuisse -pJuloto31  2>/dev/null | tail -1)
		
		##echo "type : $typeCapt"

		# Check the sensor type an SQL INSERT statement accordingly
		case $typeCapt in
			"temperature")
